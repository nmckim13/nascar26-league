...
function renderNumberStyles(data) {
  const number = String(data.assignment?.car_number || '');
  const styles = NUMBER_STYLES[number] || [];
  const selected = styles.find(style => style.key === data.numberStyleKey) || styles[0];
  document.getElementById('numberStyleCard').hidden = !styles.length;
  if (!styles.length) return;
  document.getElementById('styleCount').textContent = `${styles.length} OPTIONS FOR #${number}`;
  document.getElementById('numberStyleKey').value = selected.key;
  document.getElementById('numberStyleSource').href = selected.sourcePage;
  document.getElementById('numberStyleGrid').innerHTML = styles.map(style => `<button class="style-option${style.key === selected.key ? ' active' : ''}" type="button" data-key="${esc(style.key)}" data-image="${esc(style.image)}" data-source="${esc(style.sourcePage)}" title="${esc(style.label)}"><img src="${esc(style.image)}" alt="${esc(style.label)} number ${esc(number)}"><span>${esc(style.label)}</span></button>`).join('');
  const image = document.getElementById('carImage'); image.src = selected.image; image.alt = `${selected.label} number ${number}`;
}
function render(data) {
  ...
  const image = document.getElementById('carImage'); image.removeAttribute('src'); image.alt = '';
  ...
  renderNumberStyles(data);
}
async function init() {
  auth = await requireUser('driver.html'); if (!auth) return;
  render(await call());
  document.getElementById('numberStyleGrid').addEventListener('click', event => { const option = event.target.closest('.style-option'); if (!option) return; document.querySelectorAll('.style-option').forEach(candidate => candidate.classList.toggle('active', candidate === option)); document.getElementById('numberStyleKey').value = option.dataset.key; document.getElementById('numberStyleSource').href = option.dataset.source; document.getElementById('carImage').src = option.dataset.image; });
  document.getElementById('numberStyleForm').addEventListener('submit', async event => { event.preventDefault(); const message = document.getElementById('numberStyleMessage'); message.textContent = 'Saving…'; message.className = 'message'; try { render(await call('PATCH', Object.fromEntries(new FormData(event.currentTarget).entries()))); message.textContent = 'Number artwork saved to your driver profile.'; } catch (error) { message.textContent = error.message; message.className = 'message error'; } });
