-- Authoritative result entry, correction history, and race certification.
-- These functions are service_role-only and keep public pages read-only.

create table if not exists public.n26_result_corrections (
  ...
  result_id uuid references public.n26_race_results(id),
...
create index if not exists n26_result_corrections_race_idx
  on public.n26_result_corrections (race_id, created_at desc);

alter table public.n26_result_corrections enable row level security;
revoke all on public.n26_result_corrections from anon, authenticated;
grant all on public.n26_result_corrections to service_role;
...
begin
  ...
  if exists (
    select 1
    from jsonb_array_elements(p_results) values
  ...
  loop
    ...
    exception when invalid_text_representation then
      raise exception 'Each result needs a valid driver_id' using errcode = '22P02';
    end;
    ...
    if existing_row.id is not null then
      insert into public.n26_result_corrections (
        ...
        item || jsonb_build_object('points_earned', points_value),
    ...
    insert into public.n26_race_results (
      ...
      start_status_value,
      finish_status_value,
      finish_position_value,
      qualifying_position_value,
      qualifying_valid_value,
      pole_value,
      points_value,
      p_source_version,
      'draft'
