-- Make disqualification an explicit result status. A DQ is a start with the
-- official finish position retained for ordering, but receives zero points
-- and zero finish-quality credit. Valid qualifying still counts when started.
alter table public.n26_race_results
  drop constraint if exists n26_race_results_finish_status_check;

alter table public.n26_race_results
  add constraint n26_race_results_finish_status_check
  check (finish_status in ('classified', 'dnf', 'disqualified'));
...
begin
  ...
  if race_row.id is null then raise exception 'Race does not exist' using errcode = '23503'; end if;
  ...
  if exists (
    select 1
    from (
      select nullif(value->>'finish_position', '')::integer as finish_position
  ...
  loop
    ...
    if finish_status_value not in ('classified', 'dnf', 'disqualified') then
    ...
    if requested_seat_assignment_id is not null then
      ...
      where assignments.season_id = race_row.season_id
        ...
        and assignments.team_id = team_id_value
        and assignments.car_number = car_number_value
      order by assignments.starts_at desc limit 1;
    end if;
    ...
    if start_status_value = 'dns' then
      ...
      if finish_status_value = 'disqualified' then
        points_value := 0;
      else
    ...
    if race_row.qualifying_status = 'valid' then
      if not qualifying_valid_value or qualifying_position_value is null then
        raise exception 'Valid qualifying is required for every result' using errcode = '23514';
      end if;
      if race_row.qualifying_field_count is not null and qualifying_position_value > race_row.qualifying_field_count then
        raise exception 'Qualifying position exceeds the qualifying field' using errcode = '23514';
      end if;
    elsif qualifying_valid_value then
    ...
    if existing_row.id is not null then
      insert into public.n26_result_corrections (
        ...
        race_row.id, driver_id_value, existing_row.id, to_jsonb(existing_row),
        item || jsonb_build_object('points_earned', points_value, 'seat_assignment_id', seat_assignment_id_value),
        coalesce(item->>'correction_reason', 'Commissioner result correction'), p_corrected_by
      );
    end if;
    ...
    insert into public.n26_race_results (
      ...
      race_row.id, driver_id_value, seat_assignment_id_value, team_id_value, car_number_value,
      start_status_value, finish_status_value, finish_position_value, qualifying_position_value,
      qualifying_valid_value, pole_value, points_value, p_source_version, 'draft'
    )
...
set config = config || jsonb_build_object(
  'disqualification_policy', jsonb_build_object(
    'counts_as_start', true,
    'points', 0,
    'finish_quality_credit', 0,
    'retains_official_finish_position', true,
    'qualifying_credit_requires_start', true
  )
)
where config->>'points_schedule_status' = 'pending_24_position_extension';
